...
-(void)loadMap{
	long offset = 0;
	NSError *n;
	NSData *dataRead = [NSData dataWithContentsOfFile:mapPath];
    
	[dataRead retain];
	//NSLog(@"mapURL: %@",[n localizedDescription]);
	int lengthOfName =0;
	...
	descriptiveImage = [[UIImage alloc] initWithData:tiff];
...
//	[Environment updateDescriptiveImage:descriptiveImage];
	...
	for(i=0 ; i < numberOfAssetTypes; i++){
        ...
        if(currentArray==imageAssets){
            NSLog(@"BOWL %d", [currentArray count]);
        }
	}
...
-(void)drawMap{
    ...
    for(int i  =0 ; i < [modelAssets count] ; i++){
        [[modelAssets objectAtIndex:i] draw];
    }
...
-(id)initWithURL:(NSString*) filePath{
	if (self = [super init]){
		...
		//descriptiveImage = [UIImage imageNamed:@"defaultImage.png"];
		//[Environment updateDescriptiveImage:descriptiveImage];
